...
def get_parser_nlp(lang='en'):
    ...
    if lang != 'en':
        ...
        try:
            ...
            parser_nlp = spacy.load(
                "venv\\Lib\\site-packages\\en_core_web_sm\\en_core_web_sm-2.2.5"
            )
...
np_labels_full = {nsubj, nsubjpass, dobj, iobj, pobj, csubj, csubjpass,
                  attr}  # Probably others too
...
if __name__ == '__main__':
    doc = get_parser_nlp(
    )("With respect to all losses caused by the peril of Flood, the Company shall not be liable, in the aggregate for any one Policy year, for more than its proportionate share of US$25,000,000."
      )
    #doc = get_parser_nlp()("The Program limit of liability is US$400,000,000.")
    print(doc)
    for token in doc:
        print("{2}-{1}({3}-{6}, {0}-{5})".format(token.text, token.tag_,
                                                 token.dep_, token.head.text,
                                                 token.head.tag_, token.i + 1,
                                                 token.head.i + 1))
    print('=' * 20)
    ...
    for np in iter_nps(doc):
        print(
            f'{np} -- {np.dep_} {np.tag_}  head: {token.head.text} {token.head.dep_} {token.head.tag_}'
        )
        for t in np.subtree:
            print(f'\t{t}')
