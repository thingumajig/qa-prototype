...
try:
    stdoutdata = check_output(
        'ps aux | grep -E "nlpdemo|runner.py" | grep -v grep', shell=True)
    ...
    if "nlpdemo-app.py" in processes_str:
        st.write(
            f'<a href="http://{ip_str}:8080" target="_blank">Open NER</a>',
            unsafe_allow_html=True)

    if "run runner.py" in processes_str:
        st.write(
            f'<a href="http://{ip_str}:8501" target="_blank">Open Q&A</a>',
            unsafe_allow_html=True)

    if "run es_runner.py" in processes_str:
        st.write(
            f'<a href="http://{ip_str}:8501" target="_blank">Open Extractive summarization</a>',
            unsafe_allow_html=True)
except:
