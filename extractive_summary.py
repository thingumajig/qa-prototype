...
class ExtractiveSummary(object):
    __slots__ = [
        'text', 'sentences', 'embeddings', 'sentence_encoder', 'centroids',
        'lang'
    ]

    def __init__(self,
                 sentence_encoder_name='universal-sentence-encoder-large/5',
                 lang='en') -> None:
        super().__init__()
    ...
    def get_extractive_texts(self,
                             t,
                             threshold=0.4,
                             min_clusters_elements=3,
                             n_clusters=0,
                             minimum_sentence_len=20):
        self.preprocess_text(t)
        selected_clusters = self.cluster_embeddings(
            threshold,
            min_cluster_elements=min_clusters_elements,
...
def create_extractive_summary_gen(
        emb_name='universal-sentence-encoder-multilingual-large/3', lang='en'):
